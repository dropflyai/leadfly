...
async function setEnvironmentVariable(key, value) {
  try {
    ...
    // Use a different approach for setting env vars
    const command = `vercel env add ${key} --value="${value}" --environment=production --token ${VERCEL_TOKEN}`;
...
async function deployToProduction() {
  try {
    ...
    console.log('🌍 Live URL:', stdout.trim());
    return stdout;
  } catch (error) {
...
async function addCustomDomain() {
  try {
    ...
    console.log('⚠️  Domain configuration:', error.message);
  }
}
...
async function main() {
  ...
  try {
    ...
    // Step 4: Final production deployment with env vars
    ...
    console.log('✅ Global CDN activated');
    
    console.log('\n🌍 Your LeadFly AI is now live at:');
    console.log('   🔗 https://leadfly-ai.vercel.app (immediate access)');
    console.log('   🔗 https://leadflyai.com (after DNS setup)');
    
    console.log('\n📋 DNS Configuration Required:');
    console.log('   Add these records at your domain registrar:');
    console.log('   CNAME @ → cname.vercel-dns.com');
    console.log('   CNAME www → cname.vercel-dns.com');
    
    console.log('\n🚀 Next Steps to $2.5M ARR:');
    console.log('1. ✅ Website deployed and live');
    console.log('2. 🔄 Configure DNS (24-48 hours)');
    console.log('3. 🔐 Add authentication system');
    console.log('4. 💳 Set up Stripe billing');
    console.log('5. 👥 Launch first customers');
    console.log('6. 📈 Scale to market domination!');
    
    console.log('\n🎯 Your quantum lead generation empire is LIVE! 🎯');
